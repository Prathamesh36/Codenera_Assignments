    ...
    public static boolean isCycleDirected(ArrayList<Edge>[] graph, boolean[] vis, int curr, boolean rec[]){
        ...
        for (int i = 0; i < graph[curr].size(); i++) {
            ...
            if(rec[e.dest]){
                ...
                if(isCycleDirected(graph, vis, e.dest, rec)){   // if neighbour elements satisfies the cycle condition
                    return true;
                }
            }
        }
    ...
    public static void main(String[] args) {        //O(v+E)
        ...
        for (int i = 0; i < V; i++) {
            if (!vis[i]){
                ...
                if(isCycle){        // this condition return true when cycle is detected atleast once
